...
Indeed, it is effectively HCM but with a parallel block solving phase. pHCM differs from gHCM in that it features parallel block heaps.
...
\begin{comment}
after n iter in R^2:
	|FMM open set| <= min(3(n-1) + 4, $grid_size^2$)    (iter 0: 4, iter 1: +3, iter 2: +3, ....)
	|FIM open set| <= min($2*n^2$   , $grid_size^2$)
	|gHCM open set| <= |FIM open set|
					<= 4 | FMM open set | + (k-4) * 2
	2n^2 - 2(n-1)^2 = 2(n^2 - (n^2 - 2n + 1)) = (k, n) + 2



$s_{FIM}$ = avg \# of solves for a block using FIM
$s_{gHCM}$ = avg \# of solves for a block using gHCM
$s_{gFMM}$ = avg \# of solves for a block using gFMM
\end{comment}
